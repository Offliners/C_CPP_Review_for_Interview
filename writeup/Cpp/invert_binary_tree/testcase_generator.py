...
def flatten_tree(data, node, null_token): 
    if node is not None:
        data.append(node.val)
        flatten_tree(data, node.left, null_token) 
        flatten_tree(data, node.right, null_token) 
    else:
...
def sol(in_path, out_path):
    ...
    res = invertTree(root)
    ans = []
    flatten_tree(ans, res, int(cfg['null_token']))
    
    ans = list(filter(lambda a: a != int(cfg['null_token']), ans))
